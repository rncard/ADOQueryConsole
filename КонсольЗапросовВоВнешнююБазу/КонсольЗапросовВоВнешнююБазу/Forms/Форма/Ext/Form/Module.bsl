...
Функция Подключиться(Connection, Знач ConnectionString)
	...
	Исключение
		Сообщить(НСтр("ru='Не удалось подключиться к базе. '") + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
	Возврат Истина;
...
Процедура Отключиться(Подключение)
	...
	Попытка
		Подключение.Close();
	Исключение
		Сообщить(НСтр("ru='Ошибка при закрытии соединения. '") + ОписаниеОшибки());
	КонецПопытки;
...
Функция ВыполнитьЗапросКБазе(Знач QueryText, RecordSet, Connection)
	...
	Исключение
		Сообщить(НСтр("ru='Ошибка при выполнении запроса к базе. '") + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
	Возврат Истина;
